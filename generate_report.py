...
CHECKS = [
    ...
    ("目标预检", "IK 可达性、关节饱和与保守碰撞拒绝", "partial", "OOD 100% 拒绝；训练内仅 31/68 接受，需 phase-aware 接触语义"),
    ("低层 Joint 安全", "关节 acceleration/jerk 限制", "partial", "EEF command 合规但实际 joint jerk 仍高；下一步 Gate A.2"),
...
def render_retiming_runs(runs: list[dict]) -> str:
    ...
    for run in runs:
        ...
        filtered = run.get("filter_enabled", False)
        status = "pass" if run.get("hard_command_limits_pass") else "partial"
        label = f"{run.get('scale', 0):.2f}×" + (" + safety" if filtered else " raw reference")
        rows.append(f'''<tr><td><strong>{label}</strong></td>
        <td>{fmt(run.get("simulated_duration_s"), 2)} s</td>
        <td>{fmt(maxima.get("command_speed_m_s"), 3)}</td>
        <td>{fmt(maxima.get("command_acceleration_m_s2"), 3)}</td>
        <td>{fmt(maxima.get("command_jerk_m_s3"), 1)}</td>
        <td>{fmt(maxima.get("desired_to_filtered_position_lag_m", 0) * 1000, 1)} mm</td>
        <td>{fmt(maxima.get("actual_to_filtered_position_error_m", 0) * 1000, 1)} mm</td>
        <td><span class="pill {status}"><i></i>{"Hard limits pass" if filtered and status == "pass" else "Reference violation"}</span></td></tr>''')
    return "\n".join(rows)
...
def main() -> None:
    ...
    filtered_retiming_runs = [run for run in retiming_runs if run.get("filter_enabled")]
...
<section class="glass section"><div class="section-head"><div><h2>继续调速 Job：已知证据与信息缺口</h2><p>这里专门回答“哪些参数已经能由数据支持，哪些必须从模型作者、GPU 服务或 Unitree 控制侧取得”。</p></div><span class="pill partial"><i></i>仿真可继续 · 最终标定阻塞</span></div><div class="notice" style="padding:14px 16px;border:1px solid;border-radius:14px;margin-bottom:18px"><b>最新结论：</b> Gate A 初版已经实现并完成 0.50×–1.50× Episode-0 回放：EEF command 的 P99 v/a/j hard limits 全部通过，OOD 目标 100% 被拒绝。但实际关节 jerk 仍很高，保守碰撞规则也误拒大量训练内目标；因此它现在是可审计的 sim-only 原型，尚不能接真实 VLA 或真机。</div><div class="grid"><div class="run"><label class="muted">训练数据频率</label><strong>30 Hz</strong><small> 已确认</small></div><div class="run"><label class="muted">Action→State 延迟</label><strong>3 帧</strong><small> ≈100 ms</small></div><div class="run"><label class="muted">EEF 速度 P95</label><strong>MOTIONSPEED95 m/s</strong><small> 数据目标</small></div><div class="run"><label class="muted">关节目标速度 P95</label><strong>JOINTSPEED95 rad/s</strong><small> 非硬件上限</small></div></div></section>
...
<section class="glass section"><div class="section-head"><div><h2>当前调速参数：全部仍是工程默认值</h2><p>这些值可以做回归实验，但没有模型作者或 Unitree 安全规范背书。</p></div></div><table><thead><tr><th>参数</th><th>当前值</th><th>证据等级</th><th>还需要什么</th></tr></thead><tbody><tr><td><code>min_scale / max_scale</code></td><td>0.50× / 1.65×</td><td><span class="pill partial"><i></i>人为设定</span></td><td>允许的时间缩放范围、VLA 是否容忍重采样</td></tr><tr><td><code>near / far_distance</code></td><td>25 mm / 160 mm</td><td><span class="pill partial"><i></i>人为设定</span></td><td>任务阶段、抓取/放置精度和成功容差</td></tr><tr><td><code>max_scale_rate</code></td><td>2.0 s⁻¹</td><td><span class="pill partial"><i></i>人为设定</span></td><td>允许的加速度/jerk 及伺服跟踪带宽</td></tr><tr><td><code>max_eef_speed</code></td><td>0.65 m/s</td><td><span class="pill partial"><i></i>仅 offline 生效</span></td><td>应基于训练 P95/P99、碰撞距离和官方限制重新定值</td></tr><tr><td><code>IK max_joint_speed</code></td><td>1.2 rad/s</td><td><span class="pill partial"><i></i>非官方值</span></td><td>14 个关节逐项速度、加速度、jerk、扭矩限制</td></tr><tr><td>显式 EEF accel / jerk limit</td><td>已实现 Gate-A 原型</td><td><span class="pill pass"><i></i>5 档 hard limits 通过</span></td><td>尚未解决低层位置控制产生的实际关节 jerk</td></tr></tbody></table></section>
<section class="glass section"><div class="section-head"><div><h2>Gate A 实测：Episode 0 多档安全调速</h2><p>Raw 1.0× 作为违规参考；其余五档均经过 P99 EEF v/a/j、角速度和夹爪速度过滤。</p></div><span class="pill pass"><i></i>5/5 command limits 通过</span></div><table><thead><tr><th>模式</th><th>仿真时长</th><th>EEF v max</th><th>EEF a max</th><th>EEF jerk max</th><th>Filter lag</th><th>实际跟踪误差</th><th>结论</th></tr></thead><tbody>RETIMINGRUNS</tbody></table><div class="requirement-grid" style="margin-top:18px"><div class="phase"><em>PREFLIGHT · 训练内</em><h3>INPREPASS</h3><p>抽查 INPRETOTAL 个真实 episode 目标；INPRECOLL 因当前近似桌面/接触规则被判 collision。说明 gate 需要任务阶段和允许接触语义。</p></div><div class="phase"><em>PREFLIGHT · OOD</em><h3>OODREJECT 拒绝</h3><p>100 个宽域 OOD 双手目标全部判为 unreachable，没有下发给控制器。</p></div><div class="phase"><em>剩余硬缺口</em><h3>关节 Jerk 仍超高</h3><p>EEF command 已受限，但 MuJoCo 位置执行器/IK 仍产生最高 FILTERJOINTJERK rad/s³；必须在低层 joint command 再加一层限制。</p></div></div><p class="muted" style="margin-top:16px">所有 filtered run 最终 endpoint 误差小于 FILTERENDMAX mm；最大 desired→filtered lag 为 FILTERLAGMAX mm。接触率不作为通过项，因为当前场景和允许接触 phase 尚未精确恢复。</p></section>
...
const initial=location.hash.slice(1);if(buttons.some(b=>b.dataset.tab===initial))selectTab(initial,false);
    ...
    replacements = {
        ...
        "FILTERJOINTJERK": fmt(max(
            (run.get("maxima", {}).get("actual_joint_jerk_rad_s3", 0)
             for run in filtered_retiming_runs), default=0
        ), 0),
        "FILTERENDMAX": fmt(max(
            (run.get("endpoint_error_m", 0) * 1000 for run in filtered_retiming_runs),
            default=0,
        ), 2),
        "FILTERLAGMAX": fmt(max(
            (run.get("maxima", {}).get("desired_to_filtered_position_lag_m", 0) * 1000
             for run in filtered_retiming_runs), default=0
        ), 1),
