...
CHECKS = [
    ...
    ("跨平台复现", "GitHub Actions Ubuntu 22.04 headless", "pass", "submodule、21 项测试、OpenPI client Mock、动力学、三路 OSMesa 渲染和 HTML 检查全部通过"),
    ...
    ("Gate-A EEF 安全层", "P99 v/a/j、角速度与夹爪速度过滤", "pass", "Episode 0 的 0.50×–1.50× 五档回放均满足 command hard limits"),
    ("目标预检", "IK 可达性、关节饱和与 phase-aware 碰撞拒绝", "partial", "OOD 100% 拒绝；无 phase 标签下 free-space 为 44/68、grasp-policy 宽松视图为 59/68"),
    ("Gate-A.2 Joint command", "14 关节逐项 v/a/jerk 限制", "pass", "Episode 0 五档均满足公开训练 target P99；不是官方硬件限制"),
    ("实际 Joint 动力学", "MuJoCo qvel acceleration/jerk", "partial", "position actuator/contact 可突破 command 包络；1.25× 出现约 100k rad/s³，不能宣称动态安全"),
    ...
    ("安全约束", "速度、加速度、jerk 与不可达拒绝", "partial", "EEF/Joint command 和 preflight 已实现；实际动力学限制仍缺控制器契约与官方限制"),
    ...
    ("OpenPI Client", "WebSocket 输出审计与 fail-closed 证据", "pass", "官方 DROID schema 的 Mock protocol 通过；明确未加载神经网络且禁止 G1 执行"),
    ("远程推理", "π0.5-DROID WebSocket 与过期动作处理", "blocked", "客户端已就绪；等待 Ubuntu NVIDIA endpoint 生成 real evidence"),
    ("真实 VLA", "checkpoint 输出-only smoke", "blocked", "目标已选 pi05_droid；真实调用数仍为 0，不执行 DROID action"),
    ("本地过渡 VLA", "SmolVLA-450M + G1 16-D adapter", "todo", "已选为无 NVIDIA 时的首选实验路线；尚未下载、适配、训练或推理"),
    ("语音入口", "Whisper STT → canonical prompt", "todo", "架构已确定；当前仓库尚无麦克风、转写或指令归一化代码"),
    ("正式 G1 Policy", "π0.5 Base + G1 16-D LoRA", "blocked", "最终推荐路线；需要 NVIDIA 训练资源和自有完整 DataConfig/TrainConfig"),
...
def main() -> None:
    ...
    joint_retiming_runs = [run for run in retiming_runs if run.get("joint_filter_enabled")]
...
<section class="hero"><div><div class="eyebrow">UNITREE G1 · VLA · MUJOCO</div><h1>当前状态、技术边界与执行路线。</h1><p>按系统层、证据等级和 Gate 顺序记录项目。当前已经打通 G1 仿真与 VLA 下游执行链，但尚未运行任何真实神经 VLA；无 NVIDIA 主机时先走 SmolVLA 本地过渡路线，最终目标仍是 π0.5 Base + G1 LoRA。</p></div><div class="glass progress-card"><div class="progress-ring"><strong>PROGRESS%</strong><span>综合完成度</span></div><div class="ring-copy"><b>PASSED 项已验证</b> · PARTIAL 项部分完成 · 共 TOTAL 个门槛</div></div></section>
<section class="grid"><div class="glass metric"><label>自动测试</label><strong>TESTPASS/TESTTOTAL</strong><small>通过</small></div><div class="glass metric"><label>仿真执行链</label><strong>Ready</strong><small>G1 + Dex1</small></div><div class="glass metric"><label>M4 本地下一模型</label><strong>SmolVLA</strong><small style="color:var(--amber)">计划中</small></div><div class="glass metric"><label>真实神经 VLA 调用</label><strong>0</strong><small style="color:var(--red)">尚未运行</small></div></section>
<nav class="tabs" aria-label="报告页面"><button class="tab-btn active" data-tab="overview">执行摘要</button><button class="tab-btn" data-tab="deployment">G1 最终部署</button><button class="tab-btn" data-tab="workflow">下一步路线</button><button class="tab-btn" data-tab="deep">深度验证</button><button class="tab-btn" data-tab="retiming">调速所需信息</button><button class="tab-btn" data-tab="requirements">模型与接入</button><button class="tab-btn" data-tab="evidence">证据与历史</button></nav>
<div class="tab-panel active" data-panel="overview">
<section class="glass section"><div class="section-head"><div><h2>一页结论</h2><p>把“已经证明”“下一步选择”和“最终上线目标”严格分开。</p></div><span class="pill partial"><i></i>下游链路 Ready · Policy 未接入</span></div><div class="requirement-grid"><div class="phase"><em>01 · 当前已经可用</em><h3>G1 仿真执行底座</h3><p>官方 G1/Dex1、三路相机、16-D EEF schema、双臂 IK、碰撞预检、Gate-A/A.2 和可重复验证已经建立。</p></div><div class="phase"><em>02 · M4 过渡选择</em><h3>SmolVLA-450M</h3><p>用于建立本地“视觉 + 语言 → 16-D 动作”的真实学习链。当前只是已选方案，尚未安装、适配或训练。</p></div><div class="phase"><em>03 · 语音入口</em><h3>Whisper → Canonical Prompt</h3><p>Whisper 只负责语音转文字和指令归一化，不生成机器人动作；语音模块当前尚未接入本仓库。</p></div><div class="phase"><em>04 · 最终上线模型</em><h3>π0.5 Base + G1 LoRA</h3><p>使用同一 16-D G1 action contract 替换 SmolVLA backend；需要 NVIDIA 训练资源和我们自有的完整 config。</p></div><div class="phase"><em>05 · DROID 的角色</em><h3>只验证 OpenPI 基础设施</h3><p>π0.5-DROID 输出单臂 8-D action，只能做远程推理/时延 smoke test，绝不补零或复制后执行到 G1。</p></div><div class="phase"><em>06 · 当前证据边界</em><h3>真实神经调用仍为 0</h3><p>现有动作来自 deterministic stub 或 recorded-policy proxy；它们证明下游工程，不证明 VLA 会叠积木或真机安全。</p></div></div></section>
<section class="glass section"><div class="section-head"><div><h2>已经完成 · 按系统层</h2><p>每一层都有代码或机器可读证据，不把计划项列为完成。</p></div><span class="pill pass"><i></i>六层基础能力</span></div><table><thead><tr><th>系统层</th><th>已完成内容</th><th>主要证据</th><th>状态</th></tr></thead><tbody><tr><td><strong>仿真与视觉</strong></td><td>G1 29-DoF、官方 Dex1-1、桌面/三色方块/碰撞、三路 640×480 RGB</td><td><code>stack_scene.py</code>、camera PNG</td><td><span class="pill pass"><i></i>已验证</span></td></tr><tr><td><strong>动作与控制</strong></td><td>16-D 双手 EEF、xyzw↔wxyz、SLERP、双臂 IK、Dex1 映射</td><td>Action/IK unit tests</td><td><span class="pill pass"><i></i>已验证</span></td></tr><tr><td><strong>数据与模型审计</strong></td><td>100 episodes / 95,966 帧、约 100 ms lag、候选 EEF transform、Orbax 结构</td><td><code>*_audit.json</code></td><td><span class="pill partial"><i></i>Transform 非作者确认</span></td></tr><tr><td><strong>安全执行</strong></td><td>EEF 与 14-D joint command v/a/jerk filter、phase-aware preflight、OOD 拒绝</td><td><code>retiming_safety_validation.json</code></td><td><span class="pill partial"><i></i>Command 已验证</span></td></tr><tr><td><strong>动力学验证</strong></td><td>Episode 回放、250/250 IK、1000 OOD、30 秒稳定、随机化与扰动边界</td><td><code>comprehensive_sim_validation.json</code></td><td><span class="pill partial"><i></i>Actual jerk 待修复</span></td></tr><tr><td><strong>工程与 CI</strong></td><td>自动测试、Mock OpenPI client、HTML 历史、Ubuntu headless GitHub Actions</td><td>21/21 + CI 6/6</td><td><span class="pill pass"><i></i>已验证</span></td></tr></tbody></table></section>
<section class="glass section"><div class="section-head"><div><h2>接下来要做 · 严格按 Gate 推进</h2><p>当前不需要 Ubuntu 真机即可启动 P0–P4；每一步失败都停在仿真/离线层。</p></div><span class="pill todo"><i></i>下一项：冻结 G1 Policy Contract</span></div><div class="workflow-grid"><div class="phase"><em>P0 · 立即做</em><h3>冻结 16-D Policy Contract</h3><p>固定 observation keys、三路图像预处理、state/action 逐维语义、30 Hz、chunk horizon、absolute/delta 和 norm stats。验收：schema fixture + round-trip tests。</p></div><div class="phase"><em>P1 · 数据层</em><h3>建立 SmolVLA 数据 Adapter</h3><p>把现有 episodes 转成受版本控制的训练 manifest，按 episode 切分 train/val，禁止 frame leakage。验收：统计与原始数据一致。</p></div><div class="phase"><em>P2 · 模型可行性</em><h3>M4 加载与基线推理</h3><p>先验证 SmolVLA 依赖、MPS/CPU 内存和单帧 latency，再决定本机 LoRA 或外部训练。验收：真实 finite 16-D chunk，只离线记录。</p></div><div class="phase"><em>P3 · 语音层</em><h3>Whisper 指令归一化</h3><p>麦克风 → 转写 → 白名单 canonical prompt；低置信度、空指令和未知任务 fail-closed。验收：固定语音集 deterministic mapping。</p></div><div class="phase"><em>P4 · Gate-B</em><h3>离线 Action 审计</h3><p>真实 SmolVLA chunk 先通过 shape、finite、范围、时序、IK、碰撞和 safety checks，不执行机器人。验收：带时间戳 JSON 与轨迹可视化。</p></div><div class="phase"><em>P5 · Gate-C</em><h3>MuJoCo 闭环与迁移</h3><p>低速闭环、随机化和任务成功率通过后，再将 policy backend 替换为 π0.5 Base G1 LoRA；最后才考虑 G1 真机。</p></div></div></section>
<section class="glass section"><div class="section-head"><div><h2>可替换的目标架构</h2><p>语音、安全层和机器人接口保持不变，只替换 policy backend。</p></div></div><div class="flow"><div class="node">麦克风</div><div class="arrow">→</div><div class="node">Whisper STT</div><div class="arrow">→</div><div class="node">Canonical Prompt</div><div class="arrow">＋</div><div class="node">三路 RGB + EEF State</div><div class="arrow">→</div><div class="node">SmolVLA（当前）/ π0.5 G1 LoRA（最终）</div><div class="arrow">→</div><div class="node">16-D EEF Chunk</div><div class="arrow">→</div><div class="node">Gate-A → IK → Gate-A.2</div><div class="arrow">→</div><div class="node">MuJoCo → G1</div></div><p class="muted" style="margin-top:16px">第一轮真实 policy 测试默认关闭 adaptive retimer；先证明原速闭环，再把调速作为可选模块单独评估。</p></section>
...
<section class="glass section"><div class="section-head"><div><h2>唯一生产模型决策</h2><p>面向 Unitree G1 Education + 双 Dex1 上肢操作，选择可拥有完整训练与运行契约的模型，而不是把其他机器人 action 强行映射到 G1。</p></div><span class="pill partial"><i></i>目标已选 · 尚未训练</span></div><div class="notice" style="padding:14px 16px;border:1px solid;border-radius:14px;margin-bottom:18px"><b>最终选择：</b> <strong>OpenPI π0.5 Base + 自有 G1/Dex1 16-D 双臂 EEF LoRA</strong>。Base checkpoint 为 <code>gs://openpi-assets/checkpoints/pi05_base</code>，计划配置名为 <code>pi05_g1_edu_dual_eef</code>。这不是现成可直接上机的模型，必须先完成 G1 数据转换、LoRA、离线审计和分阶段安全验证。</div><div class="grid"><div class="run"><label class="muted">Base model</label><strong>π0.5 Base</strong><small> OpenPI</small></div><div class="run"><label class="muted">Adaptation</label><strong>LoRA</strong><small> G1/Dex1</small></div><div class="run"><label class="muted">Policy output</label><strong>16-D EEF</strong><small> 双臂绝对目标</small></div><div class="run"><label class="muted">部署形态</label><strong>Remote GPU</strong><small> G1 本地安全层</small></div></div><div class="compare"><div class="run"><h3>为什么选择它</h3><div class="row"><span>与现有 checkpoint 家族一致</span><b>π0.5 + LoRA</b></div><div class="row"><span>语言条件动作策略</span><b>原生支持</b></div><div class="row"><span>动作头可由我们定义</span><b>16-D G1 contract</b></div><div class="row"><span>已有公开示范</span><b>100 episodes</b></div><div class="row"><span>现有安全链可复用</span><b>Gate-A / IK / A.2</b></div></div><div class="run"><h3>为什么不选其他模型上线</h3><div class="row"><span>π0.5-DROID</span><b style="color:var(--red)">单臂 8-D，不兼容</b></div><div class="row"><span>SmolVLA</span><b>仅本地过渡验证</b></div><div class="row"><span>ACT</span><b>非语言原生 baseline</b></div><div class="row"><span>LGG100 现有权重</span><b style="color:var(--amber)">缺 config/transform</b></div><div class="row"><span>GR00T / OpenVLA</span><b>会引入另一套 adapter</b></div></div></div></section>
<section class="glass section"><div class="section-head"><div><h2>我们要拥有的 G1 Policy Contract</h2><p>以下内容必须在训练前写入版本控制；一旦开始训练，不允许在推理侧靠猜测改变维度、frame 或单位。</p></div><span class="pill todo"><i></i>下一交付物：g1_policy_contract.yaml</span></div><table><thead><tr><th>字段</th><th>生产定义</th><th>来源 / 说明</th><th>状态</th></tr></thead><tbody><tr><td><code>base_0_rgb</code></td><td><code>cam_left_high</code> → RGB uint8 → 固定 resize/crop</td><td>训练与 MuJoCo/真机使用同一预处理；目标分辨率拟定 224×224</td><td><span class="pill todo"><i></i>待冻结</span></td></tr><tr><td><code>left_wrist_0_rgb</code></td><td><code>cam_left_wrist</code> → RGB uint8</td><td>必须记录真机内外参、时间戳和丢帧</td><td><span class="pill todo"><i></i>待冻结</span></td></tr><tr><td><code>right_wrist_0_rgb</code></td><td><code>cam_right_wrist</code> → RGB uint8</td><td>左右相机不可交换或复制</td><td><span class="pill todo"><i></i>待冻结</span></td></tr><tr><td><code>state[16]</code></td><td>左右 EEF pose + 两个 Dex1 motor state</td><td>与 action 同一 pelvis frame、EEF site、xyzw 和单位</td><td><span class="pill partial"><i></i>候选已审计</span></td></tr><tr><td><code>prompt</code></td><td>白名单 canonical task instruction</td><td>来自语音归一化器，不允许未经审核的自由文本直接触发动作</td><td><span class="pill todo"><i></i>待实现</span></td></tr><tr><td><code>actions[:,0:3]</code></td><td>左 EEF XYZ，pelvis frame，单位 m</td><td>EEF = left_wrist_yaw_link 局部 +X 50 mm</td><td><span class="pill todo"><i></i>自有定义待冻结</span></td></tr><tr><td><code>actions[:,3:7]</code></td><td>左 EEF quaternion <code>xyzw</code></td><td>每帧归一化；MuJoCo 边界转为 wxyz</td><td><span class="pill todo"><i></i>自有定义待冻结</span></td></tr><tr><td><code>actions[:,7:14]</code></td><td>右 EEF XYZ + quaternion <code>xyzw</code></td><td>与左臂同一 pelvis frame，不做镜像复制</td><td><span class="pill todo"><i></i>自有定义待冻结</span></td></tr><tr><td><code>actions[:,14:16]</code></td><td>左/右 Dex1 motor：0 rad closed，5.5 rad open</td><td>映射方向已由公开 episode 验证</td><td><span class="pill pass"><i></i>方向已验证</span></td></tr><tr><td>时间契约</td><td>30 Hz；初始 action horizon 拟定 50</td><td>horizon 50 是 OpenPI candidate default；执行 stride/replan 必须在闭环实验后冻结</td><td><span class="pill partial"><i></i>30 Hz 已知</span></td></tr></tbody></table><p class="muted" style="margin-top:16px">因为这是我们自己的新 config，pelvis-frame / 50 mm / xyzw 将由“候选推断”升级为明确训练定义；不能把该定义反向宣称为 LGG100 作者原 config。</p></section>
<section class="glass section"><div class="section-head"><div><h2>语音机器人端到端架构</h2><p>语音识别不是 VLA。语音层决定任务文本，π0.5 根据视觉、状态和文本产生上肢动作。</p></div></div><div class="flow"><div class="node">麦克风 + Wake/Push-to-talk</div><div class="arrow">→</div><div class="node">Whisper large-v3-turbo</div><div class="arrow">→</div><div class="node">置信度 + 指令白名单</div><div class="arrow">→</div><div class="node">Canonical Prompt</div><div class="arrow">＋</div><div class="node">3 RGB + 16-D State</div><div class="arrow">→</div><div class="node">π0.5 G1 LoRA</div><div class="arrow">→</div><div class="node">16-D Chunk</div><div class="arrow">→</div><div class="node">Safety Supervisor</div><div class="arrow">→</div><div class="node">G1 Upper Body</div></div><div class="requirement-grid" style="margin-top:18px"><div class="phase"><em>VOICE · FAIL CLOSED</em><h3>只接受已训练任务</h3><p>同义中文/英文语音统一映射到训练时使用的 canonical prompt；低置信度、空文本、未知任务一律拒绝。</p></div><div class="phase"><em>VLA · UPPER BODY ONLY</em><h3>不控制腿和平衡</h3><p>第一生产版本只控制双臂与 Dex1；站立和平衡继续由 Unitree 官方低层控制器负责。</p></div><div class="phase"><em>SAFETY · LOCAL</em><h3>停止能力留在 G1 侧</h3><p>即使 policy 在远程 Ubuntu GPU，watchdog、E-stop、stale chunk、joint limits 和 hold/stop 必须在机器人本地运行。</p></div></div></section>
<section class="glass section"><div class="section-head"><div><h2>Specific Workflow · 从 Base 到 G1 Education</h2><p>每一 Gate 都产生可审计 artifact；上一 Gate 不通过，下一 Gate 不得启用执行。</p></div><span class="pill blocked"><i></i>当前位于 D0 / 基础设施准备</span></div><table><thead><tr><th>阶段</th><th>具体工作</th><th>必须产物</th><th>通过门槛</th><th>执行权限</th></tr></thead><tbody><tr><td><strong>D0 · GPU Preflight</strong></td><td>固定 OpenPI commit；用 π0.5-DROID 跑真实 output-only smoke，验证 CUDA、checkpoint、WebSocket、latency</td><td><code>openpi_droid_smoke_real.json</code></td><td>30/30 finite rank-2 8-D；仅证明基础设施</td><td><span class="pill blocked"><i></i>禁止 G1</span></td></tr><tr><td><strong>D1 · Contract</strong></td><td>建立 <code>pi05_g1_edu_dual_eef</code> TrainConfig/DataConfig、PolicyInputs/Outputs 和版本化 schema</td><td><code>g1_policy_contract.yaml</code>、golden sample、round-trip tests</td><td>逐维/frame/unit/timing 无歧义；训练/推理 transform 同源</td><td><span class="pill blocked"><i></i>禁止 G1</span></td></tr><tr><td><strong>D2 · Dataset</strong></td><td>将 100 episodes 的 raw 14 joint + 2 grip 通过自有 FK 转为 16-D EEF；按 episode 划分 train/val；补 canonical prompt</td><td>LeRobot dataset、manifest、split audit、norm stats</td><td>95,966 帧可追溯；无 frame leakage；统计与审计一致</td><td><span class="pill blocked"><i></i>禁止 G1</span></td></tr><tr><td><strong>D3 · LoRA Train</strong></td><td>从 <code>pi05_base</code> 微调；记录 commit、config、seed、GPU、step、loss 和 checkpoint hash</td><td><code>pi05_g1_edu_dual_eef</code> checkpoint + model card</td><td>验证集 action 指标稳定；无 NaN；可从干净环境恢复</td><td><span class="pill blocked"><i></i>禁止 G1</span></td></tr><tr><td><strong>D4 · Gate-B Offline</strong></td><td>真实三路 observation → policy chunk；检查 shape、finite、range、时间戳、IK、碰撞、v/a/jerk；可视化整段轨迹</td><td><code>g1_policy_smoke_real.json</code> + trajectory artifact</td><td>所有 chunk 16-D 且 fail-closed；不允许补零/复制/静默裁剪</td><td><span class="pill blocked"><i></i>只记录</span></td></tr><tr><td><strong>D5 · Gate-C MuJoCo</strong></td><td>原速、低速闭环；再测光照、方块位置/质量/摩擦、网络 jitter 和 stale chunk</td><td>批量任务报告与失败 replay</td><td>成功率、碰撞率、拒绝率和 safety intervention 达到预注册阈值</td><td><span class="pill partial"><i></i>仅仿真</span></td></tr><tr><td><strong>D6 · Shadow/HIL</strong></td><td>G1 真机只采 observation 和记录 policy 建议；控制器保持 hold；核对相机域、时延和 joint/EEF 一致性</td><td>真机 shadow log + operator sign-off</td><td>无 stale/semantic mismatch；E-stop/watchdog/limits 独立测试通过</td><td><span class="pill partial"><i></i>不执行 action</span></td></tr><tr><td><strong>D7 · Staged Robot</strong></td><td>安全支撑下单臂 free-space → 双臂 free-space → 桌面附近 → 轻物体 → 已训练任务；每阶段先低速</td><td>逐阶段安全 checklist 与 run log</td><td>人工安全员、硬件 E-stop、官方限制、低层反馈和停止测试全部通过</td><td><span class="pill todo"><i></i>分阶段授权</span></td></tr></tbody></table></section>
<section class="glass section"><div class="section-head"><div><h2>训练资源、运行拓扑与任务边界</h2><p>“部署到机器人”推荐解释为机器人系统调用 policy；第一版不要求把 3B 级模型权重直接放在 G1 机载电脑上。</p></div></div><div class="requirement-grid"><div class="phase"><em>TRAINING</em><h3>Ubuntu NVIDIA · LoRA &gt;22.5 GB VRAM</h3><p>官方估算；24 GB GPU 是实用下限，需固定 OpenPI commit 和依赖。完整微调 &gt;70 GB，不是当前选择。</p></div><div class="phase"><em>INFERENCE</em><h3>Remote GPU · &gt;8 GB VRAM</h3><p>Ubuntu policy server 经受控局域网连接；记录 P50/P95/P99、jitter、掉线和 observation age。</p></div><div class="phase"><em>ON ROBOT</em><h3>Adapter + Safety + Unitree Controller</h3><p>G1 本地保留相机/状态采集、时间同步、chunk validator、IK、limits、watchdog 和 E-stop。</p></div><div class="phase"><em>CURRENT DATA SCOPE</em><h3>只支持叠积木起点</h3><p>现有 100 episodes 不能支持开放式“语音机器人”。每个新任务必须有示范、canonical prompt、验证集和独立成功门槛。</p></div><div class="phase"><em>FIRST RELEASE</em><h3>固定站立 · 上肢操作</h3><p>先限制 workspace、物体和指令集合；不把 locomotion、全身平衡和未训练物体操作交给 VLA。</p></div><div class="phase"><em>ADAPTIVE SPEED</em><h3>最后单独启用</h3><p>首个 policy 闭环使用 1.0×/保守低速；原始闭环通过后，才评估 adaptive retimer，不与模型 bring-up 混合。</p></div></div></section>
<section class="glass section notice"><b>当前下一步：</b> 先在 Ubuntu GPU 完成 D0 的 π0.5-DROID output-only smoke；同时在代码侧冻结 <code>g1_policy_contract.yaml</code>。DROID 只验证基础设施，真正训练目标始终是 <code>pi05_base → pi05_g1_edu_dual_eef LoRA</code>。</section>
</div><div class="tab-panel" data-panel="workflow">
<section class="glass section"><div class="section-head"><div><h2>双路线推进图</h2><p>M4 本地路线现在即可推进；NVIDIA 路线以后补上，不阻塞 contract、数据和语音层开发。</p></div><span class="pill partial"><i></i>Local first · Interface stable</span></div><div class="compare"><div class="run"><h3>路线 A · 当前 M4 Air 24 GB</h3><div class="row"><span>A0</span><b>冻结 G1 16-D contract</b></div><div class="row"><span>A1</span><b>Episodes → SmolVLA adapter</b></div><div class="row"><span>A2</span><b>SmolVLA MPS/CPU feasibility</b></div><div class="row"><span>A3</span><b>LoRA 或 ACT fallback</b></div><div class="row"><span>A4</span><b>Whisper → canonical prompt</b></div><div class="row"><span>A5</span><b>Gate-B → MuJoCo closed loop</b></div></div><div class="run"><h3>路线 B · 后续 Ubuntu NVIDIA</h3><div class="row"><span>B0</span><b>π0.5-DROID output-only smoke</b></div><div class="row"><span>B1</span><b>记录 latency / horizon / stale</b></div><div class="row"><span>B2</span><b>π0.5 Base + G1 LoRA</b></div><div class="row"><span>B3</span><b>复用同一 16-D adapter</b></div><div class="row"><span>B4</span><b>重跑 Gate-B/C</b></div><div class="row"><span>B5</span><b>通过后才进入真机 Gate-D</b></div></div></div></section>
<section class="glass section"><div class="section-head"><div><h2>下一步交付物与验收标准</h2><p>不以“代码写完”为完成，而以可复现 artifact 和 fail-closed tests 为准。</p></div></div><table><thead><tr><th>优先级</th><th>交付物</th><th>验收标准</th><th>当前状态</th></tr></thead><tbody><tr><td><strong>P0</strong></td><td><code>g1_policy_contract.yaml</code> + schema fixtures</td><td>输入/输出逐维、frame、单位、频率、horizon、normalize 明确；round-trip tests 通过</td><td><span class="pill todo"><i></i>下一项</span></td></tr><tr><td><strong>P1</strong></td><td>SmolVLA dataset adapter + manifest</td><td>100 episodes 可重建；按 episode 切分；无 train/val frame leakage</td><td><span class="pill todo"><i></i>待开始</span></td></tr><tr><td><strong>P2</strong></td><td><code>smolvla_g1_smoke.json</code></td><td>本机真实模型产生 finite rank-2 16-D chunk；记录 RAM、latency、backend；G1 execution=false</td><td><span class="pill todo"><i></i>待开始</span></td></tr><tr><td><strong>P3</strong></td><td>Voice command normalizer + tests</td><td>同义语句映射到固定 prompt；低置信度/未知任务拒绝；不允许自由文本直接触发动作</td><td><span class="pill todo"><i></i>待开始</span></td></tr><tr><td><strong>P4</strong></td><td>Offline chunk audit + trajectory view</td><td>shape/finite/range/time/IK/collision/safety 全过后，才允许进入 MuJoCo</td><td><span class="pill blocked"><i></i>依赖 P2</span></td></tr><tr><td><strong>P5</strong></td><td>MuJoCo batch evaluation</td><td>预先定义抓取/堆叠成功率、碰撞率、拒绝率、latency 和 safety intervention 指标</td><td><span class="pill blocked"><i></i>依赖 P4</span></td></tr></tbody></table></section>
...
<section class="glass section"><div class="section-head"><div><h2>继续调速 Job：已知证据与信息缺口</h2><p>这里专门回答“哪些参数已经能由数据支持，哪些必须从模型作者、GPU 服务或 Unitree 控制侧取得”。</p></div><span class="pill partial"><i></i>仿真可继续 · 最终标定阻塞</span></div><div class="notice" style="padding:14px 16px;border:1px solid;border-radius:14px;margin-bottom:18px"><b>最新结论：</b> Gate A.2 已加入 14 关节逐项 P99 v/a/jerk command filter 和 phase-aware preflight；五档 command limits 均通过，训练内 grasp-policy 宽松视图为 59/68。但 1.25× 实际动力学仍出现约 100k rad/s³ jerk，因此只能证明命令整形，不证明控制器、真实 VLA 或真机动态安全。</div><div class="grid"><div class="run"><label class="muted">训练数据频率</label><strong>30 Hz</strong><small> 已确认</small></div><div class="run"><label class="muted">Action→State 延迟</label><strong>3 帧</strong><small> ≈100 ms</small></div><div class="run"><label class="muted">EEF 速度 P95</label><strong>MOTIONSPEED95 m/s</strong><small> 数据目标</small></div><div class="run"><label class="muted">关节目标速度 P95</label><strong>JOINTSPEED95 rad/s</strong><small> 非硬件上限</small></div></div></section>
<section class="glass section"><div class="section-head"><div><h2>没有加载真实 VLA，为什么还能继续？</h2><p>因为 Gate A/A.2 验证的是 policy 下游的不变量；输入源可以替换，但证据标签不能混淆。</p></div><span class="pill partial"><i></i>Recorded-policy proxy</span></div><div class="workflow-grid"><div class="phase"><em>当前输入 · 真实公开数据</em><h3>Episode-0 Recorded Actions</h3><p>Parquet 的 14-D joint action 经候选 FK 重建为 16-D EEF；它来自公开示范，不是当前 checkpoint 在线输出。</p></div><div class="phase"><em>现在能证明</em><h3>Downstream Invariants</h3><p>维度、坐标转换、插值、EEF/Joint command hard limits、IK 可达性和拒绝原因可确定性回归。</p></div><div class="phase"><em>现在不能证明</em><h3>Policy Behavior</h3><p>不能证明真实 chunk horizon、replan、latency、OOD 输出分布、视觉闭环或叠积木成功率。</p></div><div class="phase"><em>Gate B 替换输入源</em><h3>Real OpenPI Chunks</h3><p>取得 config/GPU 后，用带时间戳真实 chunk 重跑同一套 gate，并新增 stale-action 与闭环任务评测。</p></div></div></section>
<section class="glass section"><div class="section-head"><div><h2>已经从公开轨迹得到的运动包络</h2><p>跨 episode 计算且不跨 episode 边界求差分；这些是训练目标分布，不是官方安全限制。</p></div><span class="pill pass"><i></i>95,966 帧统计</span></div><table><thead><tr><th>指标</th><th>P50</th><th>P95</th><th>P99</th><th>最大值</th><th>解释</th></tr></thead><tbody><tr><td>EEF 平移速度</td><td>MOTIONSPEED50 m/s</td><td>MOTIONSPEED95 m/s</td><td>MOTIONSPEED99 m/s</td><td>MOTIONSPEEDMAX m/s</td><td class="muted">左右手取较大值</td></tr><tr><td>EEF 角速度</td><td>ANGSPEED50 rad/s</td><td>ANGSPEED95 rad/s</td><td>ANGSPEED99 rad/s</td><td>ANGSPEEDMAX rad/s</td><td class="muted">Quaternion 最短角距离</td></tr><tr><td>EEF 平移加速度</td><td>MOTIONACC50 m/s²</td><td>MOTIONACC95 m/s²</td><td>MOTIONACC99 m/s²</td><td>MOTIONACCMAX m/s²</td><td class="muted">最大值可能含目标跳变</td></tr><tr><td>EEF 平移 jerk</td><td>MOTIONJERK50 m/s³</td><td>MOTIONJERK95 m/s³</td><td>MOTIONJERK99 m/s³</td><td>MOTIONJERKMAX m/s³</td><td class="muted">用于设计滤波器，不是安全认证值</td></tr><tr><td>原始关节目标速度</td><td>JOINTSPEED50 rad/s</td><td>JOINTSPEED95 rad/s</td><td>JOINTSPEED99 rad/s</td><td>JOINTSPEEDMAX rad/s</td><td class="muted">14 关节逐帧最大值</td></tr><tr><td>原始关节目标加速度</td><td>JOINTACC50 rad/s²</td><td>JOINTACC95 rad/s²</td><td>JOINTACC99 rad/s²</td><td>JOINTACCMAX rad/s²</td><td class="muted">Gate-A.2 逐关节 P99 另见 JSON</td></tr><tr><td>原始关节目标 jerk</td><td>JOINTJERK50 rad/s³</td><td>JOINTJERK95 rad/s³</td><td>JOINTJERK99 rad/s³</td><td>JOINTJERKMAX rad/s³</td><td class="muted">训练 target，不是 actual qvel jerk</td></tr></tbody></table></section>
<section class="glass section"><div class="section-head"><div><h2>当前调速参数：全部仍是工程默认值</h2><p>这些值可以做回归实验，但没有模型作者或 Unitree 安全规范背书。</p></div></div><table><thead><tr><th>参数</th><th>当前值</th><th>证据等级</th><th>还需要什么</th></tr></thead><tbody><tr><td><code>min_scale / max_scale</code></td><td>0.50× / 1.65×</td><td><span class="pill partial"><i></i>人为设定</span></td><td>允许的时间缩放范围、VLA 是否容忍重采样</td></tr><tr><td><code>near / far_distance</code></td><td>25 mm / 160 mm</td><td><span class="pill partial"><i></i>人为设定</span></td><td>任务阶段、抓取/放置精度和成功容差</td></tr><tr><td><code>max_scale_rate</code></td><td>2.0 s⁻¹</td><td><span class="pill partial"><i></i>人为设定</span></td><td>允许的加速度/jerk 及伺服跟踪带宽</td></tr><tr><td><code>max_eef_speed</code></td><td>0.65 m/s</td><td><span class="pill partial"><i></i>仅 offline 生效</span></td><td>应基于训练 P95/P99、碰撞距离和官方限制重新定值</td></tr><tr><td><code>IK max_joint_speed</code></td><td>1.2 rad/s</td><td><span class="pill partial"><i></i>非官方值</span></td><td>14 个关节逐项速度、加速度、jerk、扭矩限制</td></tr><tr><td>显式 EEF accel / jerk limit</td><td>已实现 Gate-A 原型</td><td><span class="pill pass"><i></i>5 档 hard limits 通过</span></td><td>公开训练 target P99，非硬件限制</td></tr><tr><td>14-D Joint command v/a/jerk</td><td>已实现 Gate-A.2</td><td><span class="pill pass"><i></i>5 档逐项 limits 通过</span></td><td>实际 qvel jerk 仍受 position actuator 与接触冲击影响</td></tr></tbody></table></section>
<section class="glass section"><div class="section-head"><div><h2>Gate A.2 实测：Episode 0 多档安全调速</h2><p>依次比较 raw、仅 EEF filter，以及五档 EEF + 14-D Joint filter。表中 v/a/j 都是单次 run 最大值。</p></div><span class="pill partial"><i></i>Command 5/5 通过 · Actual dynamics 未通过</span></div><table><thead><tr><th>模式</th><th>仿真时长</th><th>EEF v/a/j</th><th>Joint command v/a/j</th><th>Actual joint jerk</th><th>最大 EEF 跟踪误差</th><th>最终误差</th><th>结论</th></tr></thead><tbody>RETIMINGRUNS</tbody></table><div class="requirement-grid" style="margin-top:18px"><div class="phase"><em>PREFLIGHT · PHASE-AWARE</em><h3>INPREPASS</h3><p>数据没有 phase 标签：free-space 视图接受 FREEPREACCEPT/INPRETOTAL；假定全部为 grasp 的宽松视图接受 INPREACCEPT/INPRETOTAL。后者仍有 INPRECOLL 个 torso/elbow contact，并使用明确的 torso/shoulder model-overlap allowlist。</p></div><div class="phase"><em>PREFLIGHT · OOD</em><h3>OODREJECT 拒绝</h3><p>100 个宽域 OOD 双手目标全部判为 unreachable；Dex1-cube 只在 grasp/place phase 放行。</p></div><div class="phase"><em>COMMAND VS ACTUAL</em><h3>JOINTCMDJERKMAX vs ACTUALJOINTJERKMAX</h3><p>Joint command jerk 最大 JOINTCMDJERKMAX rad/s³，但 actual qvel jerk 最坏 ACTUALJOINTJERKMAX rad/s³；尖峰位于 WORSTJERKJOINT、t=WORSTJERKTIME s，事件帧没有当前策略定义的 forbidden contact，指向 position actuator/controller transient。</p></div></div><p class="muted" style="margin-top:16px">五档最终 endpoint 误差最大为 FILTERENDMAX mm；最大瞬时 EEF 跟踪误差为 FILTERTRACKMAX mm。1.25× 出现动态尖峰，当前建议不把 ≥1.25× 视为候选安全档。</p></section>
...
<section class="glass section"><div class="section-head"><div><h2>最小资料包与推进门槛</h2><p>拿到以下文件即可从 sim-only 调速进入真实 VLA 调速。</p></div></div><div class="compare"><div class="run"><h3>需要模型/GPU 侧提供</h3><div class="row"><span><code>config.py</code> / config name</span><b>P0</b></div><div class="row"><span><code>transforms.py</code></span><b>P0</b></div><div class="row"><span><code>policy_timing.json</code></span><b>horizon/rate/stride</b></div><div class="row"><span><code>latency_trace.csv</code></span><b>obs/infer/action timestamps</b></div><div class="row"><span>一组真实 action chunk</span><b>含时间戳</b></div></div><div class="run"><h3>需要机器人/安全侧提供</h3><div class="row"><span><code>g1_arm_limits.yaml</code></span><b>v/a/j/torque</b></div><div class="row"><span><code>controller_contract.yaml</code></span><b>Hz/interface/watchdog</b></div><div class="row"><span><code>task_tolerances.yaml</code></span><b>grasp/place/stop</b></div><div class="row"><span><code>collision_margins.yaml</code></span><b>clearance</b></div><div class="row"><span><code>balance_limits.yaml</code></span><b>tilt/COM/contact</b></div></div></div><div class="workflow-grid" style="margin-top:18px"><div class="phase"><em>GATE A · 已完成</em><h3>EEF Safety Governor</h3><p>P99 v/a/j filter、Episode-0 五档 scale 和可审计 evidence 已完成。</p></div><div class="phase"><em>GATE A.2 · 部分完成</em><h3>Joint Command + Phase Gate</h3><p>逐关节 command limits 和 phase-aware preflight 已完成；actual dynamics 仍不合规。</p></div><div class="phase"><em>GATE B · 需 P0</em><h3>真实 VLA 仿真调速</h3><p>拿到 config、chunk timing 和 latency trace 后，接入 GPU 输出并测 stale-action。</p></div><div class="phase"><em>GATE C · 需硬件规范</em><h3>真机调速</h3><p>取得官方动态限制与低层控制契约后，才能确定最终 scale 和 stop 阈值。</p></div></div></section>
</div><div class="tab-panel" data-panel="requirements">
<section class="glass section"><div class="section-head"><div><h2>模型角色与选择</h2><p>每个模型只承担它能被证据支持的职责；没有任何现成模型可以零样本安全控制当前 G1/Dex1。</p></div><span class="pill partial"><i></i>SmolVLA 本地过渡 · π0.5 最终</span></div><table><thead><tr><th>模型</th><th>项目角色</th><th>能做什么</th><th>不能声称什么</th><th>状态</th></tr></thead><tbody><tr><td><strong>Whisper large-v3-turbo</strong></td><td>语音转文本</td><td>本地 STT、canonical prompt 输入</td><td>不生成机器人 action</td><td><span class="pill todo"><i></i>未接入</span></td></tr><tr><td><strong>SmolVLA-450M</strong></td><td>M4 本地过渡 VLA</td><td>验证视觉/语言/动作完整链和 G1 adapter</td><td>未经 G1 数据微调不能直接控制 G1</td><td><span class="pill todo"><i></i>待验证</span></td></tr><tr><td><strong>ACT</strong></td><td>资源不足时的 fallback</td><td>建立叠积木任务动作基线</td><td>不是语言原生 VLA</td><td><span class="pill todo"><i></i>备选</span></td></tr><tr><td><strong>π0.5-DROID</strong></td><td>OpenPI 远程基础设施 smoke</td><td>真实 checkpoint、WebSocket、8-D chunk 与 latency</td><td>8-D DROID action 不兼容 G1 16-D</td><td><span class="pill partial"><i></i>Client ready</span></td></tr><tr><td><strong>π0.5 Base + G1 LoRA</strong></td><td>最终 G1 policy</td><td>使用自有完整 config 训练 16-D 双臂 policy</td><td>没有 NVIDIA 训练和 Gate 验证前不能上线</td><td><span class="pill blocked"><i></i>等待资源</span></td></tr></tbody></table></section>
<section class="glass section"><div class="section-head"><div><h2>远程基础设施验证：官方 OpenPI π0.5-DROID</h2><p>以后有 Ubuntu NVIDIA 时用于打通真实神经推理、WebSocket、chunk 结构与 latency；DROID action 只记录，绝不映射到 G1。</p></div><span class="pill partial"><i></i>Client ready · GPU pending</span></div><div class="notice" style="padding:14px 16px;border:1px solid;border-radius:14px;margin-bottom:18px"><b>证据状态：</b> 当前仅完成 deterministic Mock transport，真实神经 VLA 调用仍为 <b>0</b>。Mock JSON 中 <code>neural_vla_claimed=false</code>、<code>g1_execution_enabled=false</code>。</div><div class="grid"><div class="run"><label class="muted">OpenPI pinned commit</label><strong style="font-size:16px">15a9616a0094</strong><small> 官方 main 审计</small></div><div class="run"><label class="muted">官方 config</label><strong>pi05_droid</strong><small> 完整可恢复</small></div><div class="run"><label class="muted">Mock calls</label><strong>DROIDMOCKVALID/DROIDMOCKCALLS</strong><small> protocol only</small></div><div class="run"><label class="muted">Mock chunk shape</label><strong>DROIDMOCKSHAPE</strong><small> 非神经输出</small></div></div><div class="compare" style="margin-top:18px"><div class="run"><h3>GPU server</h3><div class="row"><span>Checkpoint</span><b>pi05_droid</b></div><div class="row"><span>URI</span><b>gs://openpi-assets/checkpoints/pi05_droid</b></div><div class="row"><span>Port</span><b>8000 via SSH tunnel</b></div><div class="row"><span>启动</span><b><code>serve_policy.py --env DROID</code></b></div></div><div class="run"><h3>Output-only client</h3><div class="row"><span>入口</span><b><code>openpi_droid_smoke.py</code></b></div><div class="row"><span>Evidence</span><b>timestamp / latency / shape / finite / hash</b></div><div class="row"><span>Expected action</span><b>rank-2 · 8-D DROID</b></div><div class="row"><span>G1 execution</span><b style="color:var(--red)">HARD DISABLED</b></div></div></div><p class="muted" style="margin-top:16px">完整固定版本、SSH tunnel、安装和验收步骤见 <code>OPENPI_DROID_SMOKE.md</code>。真实 run 将单独写入 <code>results/openpi_droid_smoke_real.json</code>，不会覆盖 Mock 证据。</p></section>
...
<section class="glass section"><div class="section-head"><div><h2>GitHub Actions 跨平台证据</h2><p>六次功能提交 Workflow 均在 Ubuntu 22.04 headless 环境完成。</p></div><span class="pill pass"><i></i>6/6 成功</span></div><div class="workflow-grid"><a class="run" href="https://github.com/danielchen26/g1-vla-control/actions/runs/31452479374" target="_blank" rel="noreferrer" style="text-decoration:none;color:inherit"><h3>Run 31452479374 ↗</h3><div class="row"><span>Commit</span><b>b3c8a60</b></div><div class="row"><span>内容</span><b>深度验证代码</b></div><div class="row"><span>结论</span><b style="color:var(--green)">SUCCESS</b></div></a><a class="run" href="https://github.com/danielchen26/g1-vla-control/actions/runs/31452596513" target="_blank" rel="noreferrer" style="text-decoration:none;color:inherit"><h3>Run 31452596513 ↗</h3><div class="row"><span>Commit</span><b>a58c537</b></div><div class="row"><span>内容</span><b>CI 状态回写</b></div><div class="row"><span>结论</span><b style="color:var(--green)">SUCCESS</b></div></a><a class="run" href="https://github.com/danielchen26/g1-vla-control/actions/runs/31499342006" target="_blank" rel="noreferrer" style="text-decoration:none;color:inherit"><h3>Run 31499342006 ↗</h3><div class="row"><span>Commit</span><b>c9d7276</b></div><div class="row"><span>内容</span><b>Gate-A 安全调速</b></div><div class="row"><span>测试</span><b>15/15</b></div><div class="row"><span>结论</span><b style="color:var(--green)">SUCCESS</b></div></a><a class="run" href="https://github.com/danielchen26/g1-vla-control/actions/runs/31512781222" target="_blank" rel="noreferrer" style="text-decoration:none;color:inherit"><h3>Run 31512781222 ↗</h3><div class="row"><span>Commit</span><b>e1a4b1b</b></div><div class="row"><span>内容</span><b>Gate-A.2 Joint/Phase</b></div><div class="row"><span>测试</span><b>17/17</b></div><div class="row"><span>结论</span><b style="color:var(--green)">SUCCESS</b></div></a><a class="run" href="https://github.com/danielchen26/g1-vla-control/actions/runs/31535461865" target="_blank" rel="noreferrer" style="text-decoration:none;color:inherit"><h3>Run 31535461865 ↗</h3><div class="row"><span>Commit</span><b>e4cbb35</b></div><div class="row"><span>内容</span><b>π0.5-DROID Smoke Client</b></div><div class="row"><span>测试</span><b>21/21</b></div><div class="row"><span>结论</span><b style="color:var(--green)">SUCCESS</b></div></a><a class="run" href="https://github.com/danielchen26/g1-vla-control/actions/runs/31610777954" target="_blank" rel="noreferrer" style="text-decoration:none;color:inherit"><h3>Run 31610777954 ↗</h3><div class="row"><span>Commit</span><b>1fe757b</b></div><div class="row"><span>内容</span><b>结构化本地 VLA 路线图</b></div><div class="row"><span>测试</span><b>21/21</b></div><div class="row"><span>结论</span><b style="color:var(--green)">SUCCESS</b></div></a></div><p class="muted" style="margin-top:16px">最新 Run 通过 21 项测试、OpenPI client Mock、两组动力学、三路 OSMesa 渲染、HTML 完整性检查和 artifact 上传。</p></section>
...
const initial=location.hash.slice(1);if(buttons.some(b=>b.dataset.tab===initial))selectTab(initial,false);
    ...
    replacements = {
        ...
        "JOINTACC50": fmt(motion.get("raw_joint_target_acceleration_rad_s2", {}).get("p50"), 3),
        "JOINTACC95": fmt(motion.get("raw_joint_target_acceleration_rad_s2", {}).get("p95"), 3),
        "JOINTACC99": fmt(motion.get("raw_joint_target_acceleration_rad_s2", {}).get("p99"), 3),
        "JOINTACCMAX": fmt(motion.get("raw_joint_target_acceleration_rad_s2", {}).get("max"), 3),
        "JOINTJERK50": fmt(motion.get("raw_joint_target_jerk_rad_s3", {}).get("p50"), 2),
        "JOINTJERK95": fmt(motion.get("raw_joint_target_jerk_rad_s3", {}).get("p95"), 2),
        "JOINTJERK99": fmt(motion.get("raw_joint_target_jerk_rad_s3", {}).get("p99"), 2),
        "JOINTJERKMAX": fmt(motion.get("raw_joint_target_jerk_rad_s3", {}).get("max"), 2),
        "DROIDMOCKCALLS": str(droid_smoke_summary.get("calls", 0)),
        "DROIDMOCKVALID": str(droid_smoke_summary.get("valid_calls", 0)),
        "DROIDMOCKSHAPE": "×".join(map(str,
            (droid_smoke_summary.get("unique_action_shapes") or [["—"]])[0]
        )),
        ...
        "FILTERENDMAX": fmt(max(
            (run.get("endpoint_error_m", 0) * 1000 for run in joint_retiming_runs),
            default=0,
        ), 2),
        "FILTERTRACKMAX": fmt(max(
            (run.get("maxima", {}).get("actual_to_filtered_position_error_m", 0) * 1000
             for run in joint_retiming_runs), default=0
        ), 1),
