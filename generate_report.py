...
CHECKS = [
    ...
    ("Observation", "Three policy RGB streams", "pass", "640×480 RGB → center crop 480×480 → bilinear 224×224; shared preprocessing"),
    ...
    ("Policy", "G1-contract neural policy output", "blocked", "no neural checkpoint is verified against the frozen G1 contract"),
    ("Policy", "LGG100 eligibility", "blocked", "real-weight loader exists, but unpublished semantics mean g1_contract_verified=false"),
...
<section class="metrics"><div class="glass metric"><label>自动测试</label><strong>{tests.get('passed',0)}/{tests.get('total',0)}</strong><small>通过</small></div><div class="glass metric"><label>唯一 Contract</label><strong>16-D</strong><small>pelvis EEF</small></div><div class="glass metric"><label>G1 Contract Preflight</label><strong>{preflight.get('accepted',0)}/{preflight.get('checked',0)}</strong><small>通过</small></div><div class="glass metric"><label>G1 可执行神经 Policy</label><strong>0</strong><small>当前禁止真机</small></div></section>
<nav><button data-tab="overview" class="active">G1 摘要</button><button data-tab="contract">唯一 Contract</button><button data-tab="simulation">G1 Simulation</button><button data-tab="deployment">真机部署步骤</button><button data-tab="evidence">证据</button></nav>

<div id="overview" class="panel active">
<section class="glass section"><div class="head"><div><h2>唯一系统边界</h2><p>模拟器不是另一套机器人；它只是 G1 EDU I/O 的可替换实现。</p></div><span class="pill partial"><i></i>Sim Ready · Hardware Pending</span></div><div class="flow"><div class="node">3× RGB + 16-D State</div><div class="arrow">→</div><div class="node">G1 π0.5 LoRA</div><div class="arrow">→</div><div class="node">16-D EEF Chunk</div><div class="arrow">→</div><div class="node">Preflight + EEF Filter</div><div class="arrow">→</div><div class="node">G1 双臂 IK</div><div class="arrow">→</div><div class="node">14 Joint Filter + Dex1</div><div class="arrow">→</div><div class="node">MuJoCo / G1 EDU</div></div></section>
...
<section class="glass section"><div class="head"><div><h2>G1 EDU Production-Path Regression</h2><p>输入是 contract fixture，不伪装成 VLA；输出链与未来 G1 相同。</p></div><span class="pill {'pass' if baseline.get('success') else 'blocked'}"><i></i>{'通过' if baseline.get('success') else '失败'}</span></div><div class="grid"><div class="card rows"><h3>Baseline</h3><div class="row"><span>Preflight</span><b>{preflight.get('accepted',0)}/{preflight.get('checked',0)}</b></div><div class="row"><span>Endpoint</span><b>{fnum(base_dyn.get('endpoint_error_m'),2,1000,' mm')}</b></div><div class="row"><span>Command limits</span><b>{'PASS' if base_dyn.get('hard_command_limits_pass') else 'FAIL'}</b></div><div class="row"><span>Forbidden manipulation contact</span><b>{fnum(base_dyn.get('manipulation_contact_step_rate'),3)}</b></div></div><div class="card rows"><h3>Optional Adaptive</h3><div class="row"><span>Duration</span><b>{fnum(adapt_duration,2,suffix=' s')}</b></div><div class="row"><span>vs baseline</span><b>{fnum(duration_change,1,suffix='%')}</b></div><div class="row"><span>Endpoint</span><b>{fnum(adapt_dyn.get('endpoint_error_m'),2,1000,' mm')}</b></div><div class="row"><span>Production enabled</span><b>NO</b></div></div><div class="card rows"><h3>Hardware Blockers</h3><div class="row"><span>Actual jerk peak</span><b>{fnum(base_dyn.get('maxima',{}).get('actual_joint_jerk_rad_s3'),0,suffix=' rad/s³')}</b></div><div class="row"><span>Official limits</span><b>Missing</b></div><div class="row"><span>Low-level feedback</span><b>Missing</b></div><div class="row"><span>Hardware safety claim</span><b>NO</b></div></div></div><p class="muted">Adaptive 当前更慢，因此不进入生产默认路径。它只有在 G1-contract neural closed-loop 配对任务中改善时间且不恶化成功率、碰撞和 actual dynamics 后才可启用。</p></section>
